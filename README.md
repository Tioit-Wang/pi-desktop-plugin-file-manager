...
在 PI-Desktop 的**右侧工作面板**里浏览、查看并编辑当前项目的文件：完整目录树（图标随文件类型变化）、左右分栏（左侧文件列表可整块收起）、代码高亮编辑与保存、图片与音视频查看、CSV/JSON 结构化查看，以及新建、重命名/移动、按文件名搜索；宿主还可以让视图直接打开某个文件——包括它自己选定的项目外路径（会话临时目录 / 附件）。
...
- **宿主请求打开文件**：宿主可以要求视图打开某个文件，两条投递路径（视图入口 URL 的 `?piViewOpen=` 载荷、`view:open` 推送）都已接住，同一条请求重复投递只处理一次。项目内路径会展开并加载祖先目录，效果与点击搜索结果一致；宿主自己选定的项目外绝对路径（会话临时目录 / 附件）也能打开与保存——这类路径不在树里，因此不高亮任何行。打开前照旧走「未保存改动」确认，不会静默丢弃编辑。
...
| 限制 | 说明 |
|---|---|
| 路径包含 | 项目内路径只接受相对项目根的路径；绝对路径、`..`、以及 realpath 后落到根目录之外的**符号链接 / junction** 一律拒绝。唯一的例外是宿主请求打开的项目外绝对路径（会话临时目录 / 附件）：视图带 `external: true` 时才放行，**不做根内包含校验**（它本来就在根外），黑名单与 realpath 检查照旧全量生效 |
...
**不访问网络**：没有 `net.domains`，运行时不发起任何请求。插件数据目录里只放设置（分栏宽度、左侧列表是否收起、显示开关、Markdown 默认视图）和上面那份审计日志。
